...
- [ ] **ポイント設置・スナップ機能構想（アイデア段階、未着手。別リポジトリで開始）**: 画面上の任意の箇所にポイントを登録しておき、トラックボールをそのポイント方向へ少し転がすだけでカーソルがそこにスナップする機能。PC側常駐アプリ本体は `keyball-companion`（https://github.com/ineno771/keyball-companion 、別デバイスで開発予定）として独立リポジトリ化済み。詳細・経緯はそちらのHANDOFF.md参照。
