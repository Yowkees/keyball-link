...
- [ ] **【保留】呼吸エフェクト使用時、OLEDのスクロール分周・レイヤー表示が乱れる**: 原因調査の結果、以下が判明・対応済みだが症状は未解消。
  - 呼吸エフェクトはQMK標準では明るさ設定(val)を無視する仕様だった → 明るさ設定に従うようパッチ修正済み（`patches/0003-rgblight-breathing-respects-brightness.patch`、コミット`74feecb`+`8455233`）。明るさ調整自体は実機で効くことを確認済み。
  - ただし**明るさを下げてもOLEDの乱れ自体は解消しなかった**（電源ノイズ説は否定的）。原因は未特定（WS2812とI2Cの同時実行はQMKのAVR I2Cドライバがポーリング方式のため考えにくいことは確認済み）。ユーザー判断で**一旦保留**。次に着手する際は他の原因（例: 呼吸のタイマー更新頻度そのもの、OLED更新間隔など）を再調査すること。
- [ ] **【重要】上記EEPROM修正の実機確認（一部完了・継続中）**: 残っている確認項目:
  - 画面上の「書き込み回数」表示が書き込み成功時に+1されるか（テスト用hexで確認していたため未検証。本番配布hexに差し替えてから確認すること）
- [ ] **【重要】配布用hexの再ビルド・差し替え（未着手）**: 上記keyball-link-firmware側の一連の修正は**ソースのみ**で、`public/firmware/*.hex`はまだ旧v1.2.0のまま再ビルドされていない。OLEDの件の扱いが決まり次第、hexを作り直して差し替え、`src/lib/firmwareFeatures.ts`の`LATEST_FW_VERSION`とkeyball-link-firmware側`kb_version.h`のバージョン番号も更新すること（現状どちらも1.2.0のまま）。CHANGELOG.md更新も忘れずに。
