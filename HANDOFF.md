...
- [ ] **【保留・気が向いたら着手】呼吸エフェクト使用時、OLEDのスクロール分周・レイヤー表示が乱れる**: 原因調査の結果、以下が判明・対応済みだが症状は未解消。
  ...
  - ただし**明るさを下げてもOLEDの乱れ自体は解消しなかった**（電源ノイズ説は否定的）。原因は未特定（WS2812とI2Cの同時実行はQMKのAVR I2Cドライバがポーリング方式のため考えにくいことは確認済み）。次に着手する際は他の原因（例: 呼吸のタイマー更新頻度そのもの、OLED更新間隔など）を再調査すること。
