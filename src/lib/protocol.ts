...
// id自体はファーム側の対応表の都合上とびとびだが（6・10・12は削除済みで欠番。
// 交互点灯(10)はスプリット両ハーフで正しく動かす方式が定まらず一旦見送り。
// ※RGB_MATRIX移行版(rgb-matrix-migrationブランチ)では片側ハーフ内で完結する実装に
// 直っており10・6とも復活可能だが、このリストは本家RGBLIGHT版とも共有しているため
// 移行版がmainにマージされるまでは非表示のままにしておく）、
// 表示順は「基本の光り方→色が移り変わる系→動きのある系→季節限定」の順に整頓している。
// 11・13は季節限定エフェクト（自作の2〜3色クロスフェード。色相固定なので色相スライダーは無効）。
// 14はRGB_MATRIX移行版限定の追加エフェクト（キー反応）。本家RGBLIGHT版のファームでは
